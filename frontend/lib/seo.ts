...
export const organizationJsonLd = {
  ...
  name: SITE_NAME,
  /**
   * 옛 이름은 **표시용이 아니라 검색 별칭으로만** 남긴다.
   *
   * 2026-09-15 에 '블랭크' 에서 '블스피' 로 이름을 바꿨다. 이름 교체는 끝났다 —
   * 2026-09-21 실측에서 네이버·구글 모두 '블스피' 로 1위이고, 표시 제목도
   * '블스피 | 네이버 블로그 분석·키워드 검색' 으로 갱신돼 있었다.
   *
   * 그래도 alternateName 에서 옛 이름을 지우지 않는 이유: 이 필드는 화면에
   * 나오지 않고 "이 엔티티는 저 이름으로도 불린다"는 신호일 뿐이다. 지우면
   * 옛 이름으로 쌓은 신호가 버려지고, '블랭크' 로 찾아오던 사람이 길을 잃는다.
   */
