import { Bot, Newspaper, Search, ShoppingCart, Smartphone, TabletSmartphone, Video } from 'lucide-react'
...
export const PLATFORM_CATEGORIES: Record<PlatformCategory, { name: string; icon: LucideIcon }> = {
  search: { name: '검색 광고', icon: Search },
  social: { name: '소셜/디스플레이', icon: Smartphone },
  video: { name: '동영상 광고', icon: Video },
  native: { name: '네이티브/DSP', icon: Newspaper },
  app: { name: '앱 광고', icon: TabletSmartphone },
  commerce: { name: '커머스/쇼핑', icon: ShoppingCart },
  programmatic: { name: '프로그래매틱', icon: Bot },
}
