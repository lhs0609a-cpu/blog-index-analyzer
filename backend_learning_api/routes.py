"""
FastAPI routes for learning engine
"""
...
async def collect_learning_data(request: CollectRequest):
    """
    Collect learning data from user searches

    - Saves actual rank vs blog features
    - Automatically triggers training if threshold is met (1 sample)
    """
    try:
        current_weights = get_current_weights()
        samples_collected = 0

        # Save each search result as a learning sample
        ...
        # Auto-train if we have at least 1 sample (threshold = 1)
        all_samples = get_learning_samples(limit=1000)

        learning_triggered = False
        training_info = {}

        if len(all_samples) >= 1:  # Changed from 100 to 1
            trained, new_weights, training_info = auto_train_if_needed(
                samples=all_samples,
                current_weights=current_weights,
                min_samples=1  # Changed from 100 to 1
            )
        ...
        return {
            ...
            "message": "학습 완료! 가중치가 업데이트되었습니다." if learning_triggered else "데이터 수집 완료",
...
async def manual_train(request: TrainRequest):
    ...
    try:
        ...
        if len(samples) < 1:
            raise HTTPException(
                status_code=400,
                detail="학습 데이터가 부족합니다. 최소 1개 이상의 검색 데이터가 필요합니다."
            )
