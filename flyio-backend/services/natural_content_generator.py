"""
자연스러운 커뮤니티 콘텐츠 생성기
- 실제 사람처럼 비속어, 줄임말, 의심, 불만 등 다양한 감정 표현
- 일관된 가상 유저 페르소나
- 맥락에 맞는 대화 흐름
"""
import random
import json
import hashlib
from datetime import datetime, timedelta
from typing import List, Dict, Optional, Tuple
from dataclasses import dataclass, field
from enum import Enum
import logging

logger = logging.getLogger(__name__)


# ============ 감정 & 성격 시스템 ============

class Mood(Enum):
    """글 작성 시 기분/감정"""
    HAPPY = "happy"           # 기쁨, 성공
    FRUSTRATED = "frustrated" # 짜증, 답답
    CURIOUS = "curious"       # 궁금, 질문
    SKEPTICAL = "skeptical"   # 의심, 회의적
    EXCITED = "excited"       # 흥분, 신남
    TIRED = "tired"           # 지침, 슬럼프
    HELPFUL = "helpful"       # 도움주고 싶음
    BRAGGING = "bragging"     # 자랑, 뿌듯
    COMPLAINING = "complaining"  # 불만, 투덜
    SARCASTIC = "sarcastic"   # 비꼬는, 냉소적


class Personality(Enum):
    """유저 성격 유형"""
    FRIENDLY = "friendly"     # 친근, 이모티콘 많이
    BLUNT = "blunt"          # 직설적, 단답
    DETAILED = "detailed"     # 꼼꼼, 장문
    CASUAL = "casual"        # 편한, 반말 섞임
    POLITE = "polite"        # 공손, 존댓말
    SARCASTIC = "sarcastic"  # 비꼬는, 냉소적


# ============ 자연스러운 표현 사전 ============

# 줄임말 & 신조어
SLANG_DICT = {
    "정말": ["진짜", "ㄹㅇ", "레알", "마자", "ㅇㅈ"],
    "대박": ["ㄷㅂ", "대박ㅋㅋ", "헐", "미쳤다", "오..."],
    "감사합니다": ["감사해요", "ㄳㄳ", "고마워요", "땡큐", "쌩유"],
    "축하합니다": ["축하해요", "ㅊㅋㅊㅋ", "오 축하!", "ㅊㅋ"],
    "힘내세요": ["화이팅", "ㅎㅇㅌ", "힘내요", "파이팅", "홧팅"],
    "모르겠어요": ["모르겠음", "모름ㅠ", "???", "뭐지", "모르겟어요"],
    "좋아요": ["좋음", "굿", "ㄱㅇㄷ", "좋네요", "괜찮네"],
    "그런데": ["근데", "ㄱㄷ", "그치만", "헌데"],
    "어떻게": ["어케", "어캐", "어떡게", "어떻게요"],
    "왜": ["왜요", "왜??", "웨", "왜지"],
}

# 감탄사 & 필러
FILLERS = {
    "start": ["아", "음", "어", "흠", "오", "앗", "엥", "헐", "와", "우와"],
    "mid": ["그", "이제", "뭐", "좀", "막", "약간", "걍", "그냥"],
    "end": ["ㅋㅋ", "ㅎㅎ", "ㅠㅠ", "...", ";;;", "ㄷㄷ", "??", "!!", "~"],
}

# 이모티콘 (성격별 빈도 다름)
EMOTICONS = {
    "positive": ["😊", "🎉", "💪", "✨", "🔥", "👍", "😄", "🥳", "💕", "☺️"],
    "negative": ["😢", "😭", "ㅠㅠ", "ㅜㅜ", "😞", "💔", "😤", "😠"],
    "neutral": ["🤔", "😅", "ㅎㅎ", "ㅋㅋ", "👀", "🙄", "😶"],
}

# 비속어/강한 표현 (가벼운 수준)
MILD_EXPLETIVES = [
    "ㅅㅂ", "시발", "ㅈㄴ", "존나", "ㄹㅇㅋㅋ", "ㅁㅊ", "미친",
    "개", "존", "레전드", "ㄹㅇ루다가", "헐ㅋㅋㅋ",
    "아 진짜", "아ㅋㅋ", "에휴", "하...", "아놔",
]

# 의심/회의적 표현
SKEPTICAL_EXPRESSIONS = [
    "진짜요?", "에이 설마", "그게 되나요?", "글쎄요...",
    "믿기 힘든데", "뻥 아님?", "광고 아니죠?", "홍보글 같은데",
    "진짠지 모르겠네", "의심됨ㅋㅋ", "어 그래요?", "흠...",
    "근데 이거 효과 있음?", "저도 해봤는데 안되던데",
    "사람마다 다른 거 아님?", "운 좋았던 거 아닐까",
]

# 불만/투덜 표현
COMPLAINT_EXPRESSIONS = [
    "아 짜증나", "왜 안되지", "ㅠㅠ 힘들다", "하...",
    "이게 뭐야", "어이없네", "답답해", "미치겠다",
    "포기하고 싶음", "의욕 없어", "귀찮아", "힘듦",
    "이거 언제까지 해야함?", "진짜 안됨ㅠㅠ", "나만 안되나",
]


# ============ 가상 유저 시스템 ============

@dataclass
class VirtualUser:
    """일관된 성격을 가진 가상 유저"""
    id: int
    name: str
    personality: Personality
    blog_months: int  # 블로그 경력 (개월)
    skill_level: str  # Bronze, Silver, Gold, Platinum
    main_topic: str  # 주 관심 주제
    writing_style: Dict = field(default_factory=dict)

    def __post_init__(self):
        # 성격에 따른 글쓰기 스타일 설정
        self.writing_style = {
            "uses_slang": self.personality in [Personality.CASUAL, Personality.BLUNT],
            "uses_emoticons": self.personality in [Personality.FRIENDLY, Personality.CASUAL],
            "emoticon_frequency": 0.7 if self.personality == Personality.FRIENDLY else 0.3,
            "uses_honorifics": self.personality in [Personality.POLITE, Personality.DETAILED],
            "avg_length": "long" if self.personality == Personality.DETAILED else "short" if self.personality == Personality.BLUNT else "medium",
            "uses_expletives": self.personality in [Personality.CASUAL, Personality.BLUNT, Personality.SARCASTIC],
            "expletive_frequency": 0.1 if self.personality == Personality.CASUAL else 0.05,
        }


# 가상 유저 이름 풀 (더 자연스럽게)
USER_NAME_COMPONENTS = {
    "prefixes": [
        "", "작은", "큰", "오늘의", "매일", "소소한", "행복한", "꾸준한",
        "열심히", "초보", "프로", "도전", "성장", "시작", "노력",
    ],
    "cores": [
        "민지", "수현", "지유", "하은", "서연", "예린", "소민", "채원",
        "준호", "민석", "지훈", "현우", "성민", "재현", "도윤", "시우",
        "블로거", "기록", "일상", "하루", "생활", "이야기", "스토리",
        "맘", "대디", "워킹맘", "직장인", "프리랜서", "대학생", "취준생",
    ],
    "suffixes": [
        "", "log", "diary", "story", "_", ".", "님", "씨",
        "92", "95", "98", "00", "01", "88", "90",
        "_blog", "_daily", "_life", "의하루", "의일상",
    ],
}

# 주제 풀
TOPICS = [
    "맛집", "카페", "여행", "육아", "인테리어", "패션", "뷰티", "IT",
    "재테크", "자기계발", "운동", "요리", "반려동물", "독서", "영화",
    "일상", "리뷰", "체험단", "애드센스", "N잡",
    "캠핑", "등산", "수영", "다이어트", "홈카페", "원예", "수공예",
    "사진", "브이로그", "공부법", "어학", "자격증", "부동산",
]


def generate_virtual_user(user_id: int = None) -> VirtualUser:
    """가상 유저 생성 (ID 기반으로 일관된 성격 유지)"""
    if user_id is None:
        user_id = random.randint(10000, 99999)

    # ID를 시드로 사용하여 일관된 랜덤값 생성
    seed = int(hashlib.md5(str(user_id).encode()).hexdigest()[:8], 16)
    rng = random.Random(seed)

    # 이름 생성
    prefix = rng.choice(USER_NAME_COMPONENTS["prefixes"])
    core = rng.choice(USER_NAME_COMPONENTS["cores"])
    suffix = rng.choice(USER_NAME_COMPONENTS["suffixes"])
    name = f"{prefix}{core}{suffix}".strip()

    # 성격 (가중치 적용)
    personalities = list(Personality)
    weights = [25, 15, 20, 25, 10, 5]  # friendly, blunt, detailed, casual, polite, sarcastic
    personality = rng.choices(personalities, weights=weights)[0]

    # 블로그 경력 (지수 분포 - 초보가 많음)
    blog_months = min(int(rng.expovariate(0.1)), 36)

    # 스킬 레벨 (경력에 비례)
    if blog_months < 2:
        skill_level = "Bronze"
    elif blog_months < 6:
        skill_level = rng.choice(["Bronze", "Silver"])
    elif blog_months < 12:
        skill_level = rng.choice(["Silver", "Gold"])
    else:
        skill_level = rng.choice(["Gold", "Platinum"])

    return VirtualUser(
        id=user_id,
        name=name,
        personality=personality,
        blog_months=blog_months,
        skill_level=skill_level,
        main_topic=rng.choice(TOPICS),
    )


# ============ 자연스러운 텍스트 변환기 ============

class NaturalTextTransformer:
    """텍스트를 더 자연스럽게 변환"""

    @staticmethod
    def add_typos(text: str, probability: float = 0.02) -> str:
        """자연스러운 오타 추가"""
        typo_map = {
            "ㅏ": "ㅓ", "ㅓ": "ㅏ", "ㅗ": "ㅜ", "ㅜ": "ㅗ",
            "ㄱ": "ㅋ", "ㄷ": "ㅌ", "ㅂ": "ㅍ",
        }
        result = list(text)
        for i, char in enumerate(result):
            if random.random() < probability and char in typo_map:
                result[i] = typo_map[char]
        return "".join(result)

    @staticmethod
    def apply_slang(text: str, intensity: float = 0.3) -> str:
        """줄임말/신조어 적용"""
        result = text
        for formal, slangs in SLANG_DICT.items():
            if formal in result and random.random() < intensity:
                result = result.replace(formal, random.choice(slangs), 1)
        return result

    @staticmethod
    def add_fillers(text: str, user: VirtualUser) -> str:
        """감탄사/필러 추가"""
        if user.personality not in [Personality.CASUAL, Personality.FRIENDLY]:
            return text

        sentences = text.split(".")
        result = []

        for i, sentence in enumerate(sentences):
            if not sentence.strip():
                continue

            # 시작에 필러 추가 (20% 확률)
            if i == 0 and random.random() < 0.2:
                filler = random.choice(FILLERS["start"])
                sentence = f"{filler} {sentence.strip()}"

            # 중간에 필러 추가 (10% 확률)
            if random.random() < 0.1:
                words = sentence.split()
                if len(words) > 3:
                    insert_pos = random.randint(1, len(words) - 1)
                    filler = random.choice(FILLERS["mid"])
                    words.insert(insert_pos, filler)
                    sentence = " ".join(words)

            result.append(sentence)

        return ".".join(result)

    @staticmethod
    def add_emoticons(text: str, user: VirtualUser, mood: Mood) -> str:
        """이모티콘 추가"""
        if not user.writing_style.get("uses_emoticons"):
            return text

        freq = user.writing_style.get("emoticon_frequency", 0.3)

        # 기분에 따른 이모티콘 선택
        if mood in [Mood.HAPPY, Mood.EXCITED, Mood.BRAGGING]:
            emoticon_pool = EMOTICONS["positive"]
        elif mood in [Mood.FRUSTRATED, Mood.TIRED, Mood.COMPLAINING]:
            emoticon_pool = EMOTICONS["negative"]
        else:
            emoticon_pool = EMOTICONS["neutral"]

        # 문장 끝에 이모티콘 추가
        sentences = text.split("\n")
        result = []

        for sentence in sentences:
            if sentence.strip() and random.random() < freq:
                emoticon = random.choice(emoticon_pool)
                sentence = f"{sentence.rstrip()} {emoticon}"
            result.append(sentence)

        return "\n".join(result)

    @staticmethod
    def add_line_breaks(text: str, user: VirtualUser) -> str:
        """자연스러운 줄바꿈"""
        if user.personality == Personality.BLUNT:
            # 짧게 끊어서
            return text.replace(". ", ".\n")
        elif user.personality == Personality.DETAILED:
            # 문단 단위로
            sentences = text.split(". ")
            result = []
            for i, s in enumerate(sentences):
                result.append(s)
                if (i + 1) % 3 == 0:
                    result.append("\n")
            return ". ".join(result)
        return text


# ============ 콘텐츠 생성기 (무드별) ============

# 게시글 템플릿 (무드별로 세분화)
POST_TEMPLATES_BY_MOOD = {
    Mood.HAPPY: {
        "success": [
            {
                "title": "{keyword} 드디어 {rank} 찍었다!!",
                "content": """ㅋㅋㅋㅋ 드디어 됐다

{keyword}로 {rank} 진입함

블로그 {months}개월만인데 {struggle} 드디어 되네

{method}

ㄹㅇ 꾸준히 하면 되긴 하는듯""",
            },
            {
                "title": "오늘 확인했는데 {keyword} 상위노출 됨",
                "content": """아까 검색해봤는데 {rank} 찍혀있음

{struggle} 근데 됐네 ㅋㅋ

{method}

이제 다음 키워드 도전해봐야지""",
            },
            {
                "title": "방금 상위노출 확인했는데 손떨림",
                "content": """{keyword} 어제 올린 글인데
아침에 검색해봤더니 {rank} 찍혀있음

{months}개월 하면서 처음이라 ㄹㅇ 떨림

{method}

기분 진짜 좋다""",
            },
            {
                "title": "{keyword} 성공 후기 남깁니다",
                "content": """솔직히 안될줄 알았는데 진짜 됐어요

{months}개월 걸렸는데 {method}

비결이라면 {method_name}

이것만 하면 진짜 차이남""",
            },
        ],
        "tip": [
            {
                "title": "{topic} 효과본 방법 공유할게요",
                "content": """블로그 {months}개월차인데 제가 실제로 해본 방법이에요

{tip_content}

이거 {days} 정도 해보니까 확실히 달라졌어요

질문 있으시면 댓글로~""",
            },
            {
                "title": "이렇게 하니까 {topic} 바로 효과남",
                "content": """요즘 제가 쓰는 방법인데

{tip_content}

처음에는 반신반의했는데 {days}만에 결과가 보이더라

아직 안해보신 분들 꼭 해보세요""",
            },
            {
                "title": "블로그 {months}개월차가 깨달은 {topic}",
                "content": """초보때 이거 알았으면 시간 아꼈을텐데 싶은 것들

{tip_content}

저도 처음에는 몰랐는데 이거 하고 나서 확 달라졌음""",
            },
            {
                "title": "{topic} 꿀팁 하나만 공유합니다",
                "content": """많은 분들이 놓치고 있는건데

{tip_content}

이거 하나만 바꿔도 결과가 확 달라져요""",
            },
        ],
    },
    Mood.FRUSTRATED: {
        "question": [
            {
                "title": "아 왜 상위노출 안되냐",
                "content": """진짜 {days}째 매일 올리는데 왜 안됨?

{stats}

뭐가 문제인지 모르겠음
고수분들 좀 알려주세요 ㅠㅠ""",
            },
            {
                "title": "{keyword} 진짜 어렵네...",
                "content": """이 키워드 경쟁 너무 심한거 아님?

{attempt_content}

나만 안되는건가 아님 원래 이런건가""",
            },
            {
                "title": "똑같이 했는데 왜 나만 안돼?",
                "content": """진짜 이해가 안됨

상위 블로그 따라했거든?
글자수도 맞추고 사진 수도 맞추고

근데 나는 5페이지 ㅋㅋ
{months}개월째 이러고 있음

대체 뭐가 다른건지 아시는분?""",
            },
            {
                "title": "갑자기 순위 빠졌는데 이유가 뭘까",
                "content": """{keyword} 원래 {rank}였는데

오늘 확인하니까 아예 안보임

바꾼거 하나도 없는데 왜 이러지?
{complaint}""",
            },
            {
                "title": "{keyword} 때문에 멘탈 나감",
                "content": """{attempt_content}

이거 진짜 내 블로그 문제인건가
아님 키워드 자체가 어려운건가

솔직히 포기하고 싶음""",
            },
        ],
        "free": [
            {
                "title": "블로그 하기 싫다",
                "content": """요즘 의욕이 안남

{months}개월 했는데 성과가 없으니까...

{complaint}

그래도 해야되나 하""",
            },
            {
                "title": "오늘 조회수 보고 현타옴",
                "content": """3시간 걸려서 쓴 글 조회수 5

{complaint}

진짜 누구를 위해 쓰는건지 모르겠음
그냥 끄적여봄""",
            },
            {
                "title": "블로그 현실 ㅋㅋㅋ",
                "content": """기대: 상위노출 달성 후 방문자 폭증
현실: {complaint}

{months}개월차의 현실이 이렇습니다
그래도 하긴 해야되니까""",
            },
        ],
    },
    Mood.SKEPTICAL: {
        "question": [
            {
                "title": "이거 진짜 효과 있는거 맞음?",
                "content": """어디서 {method_name} 하면 된다고 하던데

근데 {doubt}

해보신 분 있음? 솔직한 후기 좀""",
            },
            {
                "title": "ㄹㅇ 궁금한건데 {topic} 의미있음?",
                "content": """{doubt}

저만 이렇게 생각하나?
실제로 효과 본 분 있으면 알려주세요

증거 없으면 안믿을거임""",
            },
            {
                "title": "상위노출 성공했다는 분들 진짜임?",
                "content": """커뮤니티 보면 매일 성공 후기 올라오는데

{skeptical_points}

의심병인건지 합리적 의심인건지""",
            },
        ],
        "free": [
            {
                "title": "솔직히 이해 안되는 것들",
                "content": """블로그 커뮤니티 보다보면

{skeptical_points}

나만 그런가?""",
            },
            {
                "title": "블로그 강의 들으면 진짜 됨?",
                "content": """주변에서 블로그 강의 듣고 성공했다는데

{doubt}

수십만원 내고 듣는 강의가 무료 정보랑 뭐가 다름?
들어보신 분 솔직후기 부탁""",
            },
            {
                "title": "광고글인지 진짜 후기인지 구분이 안됨",
                "content": """요즘 후기 보면

다 좋다고만 하는데 {doubt}

솔직한 단점도 얘기해주는 사람이 별로 없음""",
            },
        ],
    },
    Mood.CURIOUS: {
        "question": [
            {
                "title": "{keyword} 지금 도전하면 가능할까?",
                "content": """키워드 분석해봤는데

{stats}

내 지수로 가능할까요?
비슷한 수준이셨던 분들 어떠셨어요?""",
            },
            {
                "title": "하루에 포스팅 몇 개가 적당함?",
                "content": """요즘 {frequency}개씩 올리고 있는데

너무 많이 올리면 저품질 걸린다는 말도 있고
적게 올리면 지수 안오른다는 말도 있고

다들 어떻게 하세요?""",
            },
            {
                "title": "블로그 시작 {months}개월차인데 이 정도면 괜찮은건가?",
                "content": """현재 {stats}

다른 분들은 이때쯤 어느 정도였어요?
비교할 기준이 없어서

잘 하고 있는건지 모르겠음""",
            },
            {
                "title": "{topic} 어떻게 배웠어요?",
                "content": """블로그 시작한지 {months}개월인데

{topic}을 잘 모르겠어요
유튜브 보면 다 말이 다르고

실제로 하시는 분들은 어디서 배우셨어요?""",
            },
            {
                "title": "{keyword} 경쟁도 어느정도예요?",
                "content": """이 키워드로 도전해볼까 하는데

현재 내 상태는 {stats}

이 키워드가 나한테 맞는 난이도인지 감이 안와요
비슷한 키워드 해보신 분?""",
            },
            {
                "title": "체험단 vs 자체 포스팅 뭐가 나음?",
                "content": """요즘 고민되는게

체험단 위주로 가면 콘텐츠는 풍부한데 자유도가 없고
내가 쓰고싶은거 쓰면 글감이 한정적이고

{months}개월차 기준으로 어디에 집중하는게 나을까요""",
            },
        ],
    },
    Mood.HELPFUL: {
        "tip": [
            {
                "title": "{topic} 정리해봤어요 (초보분들 참고)",
                "content": """저도 초보때 이거 모르겠었는데 정리해봄

{tip_content}

도움 됐으면 좋겠어요""",
            },
            {
                "title": "블랭크 이렇게 활용하면 진짜 편함",
                "content": """블랭크 쓰시는 분들 참고하세요

{tip_content}

저는 이렇게 쓰고 있는데 괜찮더라구요""",
            },
            {
                "title": "{months}개월 해보고 깨달은 것들 정리",
                "content": """블로그 {months}개월 하면서 삽질도 많이 했는데
그러면서 깨달은 것들 공유할게요

{tip_content}

초보분들한테 도움이 되면 좋겠어요""",
            },
            {
                "title": "초보 때 알았으면 좋았을 {topic}",
                "content": """저 블로그 {months}개월차인데
처음에 이거 몰라서 시간 많이 낭비했어요

{tip_content}

이것만 알아도 시행착오 줄일 수 있어요""",
            },
            {
                "title": "{topic} 실전에서 쓰는 방법 알려드림",
                "content": """이론은 많은데 실전 방법은 잘 안알려주더라

제가 실제로 하는 방법 공유할게요

{tip_content}

다들 해보시고 결과 공유해주세요""",
            },
        ],
    },
    Mood.BRAGGING: {
        "success": [
            {
                "title": "ㅋㅋㅋ 드디어 {achievement}",
                "content": """드디어 {achievement} 함

{details}

뿌듯하다 ㅎㅎ

다음 목표는 {next_goal}""",
            },
            {
                "title": "{achievement} 인증합니다",
                "content": """{months}개월 걸렸지만 드디어

{achievement} 달성!!

{details}

노력은 배신하지 않네요""",
            },
            {
                "title": "목표 달성 기념 글",
                "content": """오늘 {achievement} 달성했습니다

{details}

처음 블로그 시작할때 {months}개월 안에 하겠다고 했는데
진짜 됐네

다음은 {next_goal} 가보자""",
            },
            {
                "title": "자랑 좀 할게요 ㅎㅎ",
                "content": """ㅋㅋ 자랑하려고 글 씀

{achievement} 해냈습니다

{details}

이 기분 아시는분 공감해주세요""",
            },
        ],
    },
    Mood.COMPLAINING: {
        "free": [
            {
                "title": "네이버 알고리즘 진짜 이해 안됨",
                "content": """{complaint}

진짜 기준이 뭔지 모르겠음

같은 조건인데 왜 어떤 글은 되고 어떤 글은 안되냐""",
            },
            {
                "title": "요즘 상위노출 너무 어려워진듯",
                "content": """예전엔 좀 쉬웠던것 같은데

요즘은 {complaint}

다들 그럼? 나만 그런가""",
            },
            {
                "title": "블로그 하면서 제일 짜증나는 것",
                "content": """솔직히 {complaint}

이거 말고도 짜증나는게
시간 투자 대비 성과가 너무 안보임

다들 어떻게 버팀?""",
            },
            {
                "title": "아니 이게 말이 되냐",
                "content": """대충 쓴 글이 상위노출 되고
정성들여 쓴 글이 묻히는거 진짜 뭐냐

{complaint}

알고리즘 이해하는 사람 있음?""",
            },
        ],
    },
    Mood.TIRED: {
        "free": [
            {
                "title": "슬럼프 온듯",
                "content": """요즘 글 쓰기가 너무 힘듦

{tired_content}

어떻게 극복하셨어요? 조언 좀 부탁""",
            },
            {
                "title": "블로그 쉬어도 될까요",
                "content": """{months}개월째 쉬지 않고 했는데
진짜 지침

{tired_content}

좀 쉬면 지수 떨어질까봐 걱정이 되는데
쉬어본 분들 어땠어요?""",
            },
            {
                "title": "매일 쓰는게 맞는건지 모르겠음",
                "content": """{tired_content}

매일 1포 해야 된다길래 억지로 쓰고 있는데
이러면 퀄리티만 떨어지는것 같고

쉬엄쉬엄 해도 되나요?""",
            },
            {
                "title": "번아웃 왔어요",
                "content": """블로그 {months}개월차인데 번아웃 심하게 옴

{tired_content}

예전에는 글 쓰는게 재밌었는데
이젠 의무감으로만 하고 있음""",
            },
        ],
    },
    Mood.EXCITED: {
        "success": [
            {
                "title": "아 진짜 대박 {keyword} {rank}!!!!",
                "content": """미쳤다 미쳤다 미쳤다

{keyword} {rank} 찍힘!!!!

{months}개월만에 드디어 해냈다

{method}

아 기분 너무 좋아서 소리지름""",
            },
            {
                "title": "이건 진짜 자랑해도 되죠?!",
                "content": """블로그 시작하고 제일 기쁜 날

{achievement} 달성!!

{details}

진짜 눈물날뻔 했음 ㅋㅋㅋ""",
            },
        ],
        "free": [
            {
                "title": "오늘 기분 너무 좋아서 글 씀",
                "content": """{achievement} 달성한 기념으로

오늘 하루종일 기분이 좋음

{details}

여러분도 포기하지 마세요 진짜 되긴 됩니다""",
            },
        ],
    },
    Mood.SARCASTIC: {
        "free": [
            {
                "title": "블로그 고수들의 공통 조언 ㅋㅋ",
                "content": """커뮤니티 돌아다니면서 정리함

1. 꾸준히 하세요 (= 답을 모르겠음)
2. 좋은 글 쓰세요 (= 구체적인 방법은 몰라요)
3. 키워드 분석하세요 (= 블랭크 쓰세요)

이거 말고 진짜 도움되는 조언 아시는분""",
            },
            {
                "title": "블로그 시작 n개월차의 공통 증상",
                "content": """1개월: 나도 할 수 있다!
3개월: 왜 안되지?
6개월: {complaint}
12개월: 그래도 해야지...

맞지? ㅋㅋㅋ""",
            },
            {
                "title": "이것만 하면 상위노출 된다는데요",
                "content": """어디서 주워들은 꿀팁

{method_name}

이거 하면 무조건 된대요 여러분
안되면 그건 여러분 탓이래요

ㅎㅎ 네 그렇답니다""",
            },
        ],
    },
}

# 댓글 템플릿 (무드별)
COMMENT_TEMPLATES_BY_MOOD = {
    Mood.HAPPY: [
        "오 축하해요!!", "ㅊㅋㅊㅋ", "대박 부럽다", "와 진짜?? 멋있어요",
        "저도 이렇게 되고 싶음ㅠ", "화이팅!", "좋은 정보 감사해요~",
        "우와 대단하시다 진짜", "어떻게 하신거예요?", "저도 도전해볼게요!",
        "비결이 뭐예요?", "오 너무 좋다", "진짜 부럽ㅠㅠ 저도 열심히 해야겠다",
        "이거 보고 동기부여 받았어요", "멋져요! 계속 화이팅이에요",
    ],
    Mood.FRUSTRATED: [
        "저도요 ㅠㅠ", "공감...", "힘내세요 ㅠ", "다들 그래요",
        "포기하지 마세요!", "저도 그랬는데 어느순간 되더라구요",
        "저도 지금 같은 상황이에요ㅠ", "공감 백만번", "아 진짜 힘들죠",
        "같이 힘내요!", "저도 비슷한 경험 있어서 공감됨",
        "시간이 좀 걸려도 결국 되더라구요", "어떤 키워드인지 알려주시면 봐드릴게요",
        "혹시 글 링크 공유 가능해요? 피드백 해드릴게요", "조급하게 생각하지 마세요",
    ],
    Mood.SKEPTICAL: [
        "저도 좀 의문임", "글쎄요...", "해봐야 알듯", "사바사 아닐까요",
        "저는 효과 없었음", "운도 있는것 같아요",
        "맞아요 이거 나도 좀 의심스러웠음", "확실한 근거가 있어야 믿지",
        "저도 비슷하게 생각해요", "사람마다 다른 거 아닐까",
        "직접 해봐야 아는거죠", "너무 맹신하면 안될듯",
        "저는 해봤는데 반반이었어요", "케바케인것 같아요",
    ],
    Mood.CURIOUS: [
        "저도 궁금!", "알려주세요~", "오 그런가요?", "해보셨어요?",
        "결과 공유해주세요!", "저도 알고싶어요",
        "오 이거 궁금했는데", "저도 같은 고민이에요", "답변 달리면 알려주세요ㅋㅋ",
        "이거 나도 알고싶다", "혹시 결과 나오면 공유해주실수 있어요?",
        "비슷한 상황인데 저도 궁금하네요", "대댓에 답변 달아주세요!",
        "와 이 질문 진짜 공감됨", "아시는분 답변좀요~",
    ],
    Mood.HELPFUL: [
        "감사합니다!", "도움됐어요!", "오 이거 몰랐네", "꿀팁이다",
        "저장해둘게요", "나중에 참고할게요!",
        "와 이거 진짜 유용하다", "바로 적용해볼게요", "정리 잘 해주셨네요",
        "이런 글이 진짜 도움됨", "감사합니다 바로 시도해봄", "꿀팁 감사해요!",
        "이거 보고 바로 수정했어요", "초보한테 진짜 필요한 내용이네요",
        "이 글 즐겨찾기 합니다", "아 이래서 그랬구나 이제 이해됐어요",
    ],
    Mood.SARCASTIC: [
        "ㅋㅋㅋㅋ", "그런가요~", "네네", "아 그렇구나",
        "흠...", "글쎄요", "모르겠네요",
        "ㅇㅇ 맞아요 맞아요", "와 대단하시네요", "그렇군요 ㅋ",
        "오 그래요?", "하하 공감이네요", "아 네...",
    ],
}

# 변수 데이터
TEMPLATE_DATA = {
    "keyword": [
        "강남 맛집", "서울 카페", "부산 여행", "제주 숙소", "홍대 맛집",
        "성수동 카페", "인테리어 팁", "다이어트 식단", "육아 꿀팁", "자취 요리",
        "노트북 추천", "에어팟 리뷰", "영양제 추천", "운동 루틴", "독서 추천",
        "강남 브런치", "이태원 파스타", "판교 점심", "해운대 숙소", "경주 당일치기",
        "아기 이유식", "수분크림 추천", "에어프라이어 레시피", "원룸 인테리어",
        "쿠팡 추천템", "아이패드 활용법", "코딩 독학", "주식 초보", "적금 비교",
        "캠핑 장비", "강아지 간식", "고양이 사료", "헬스 초보", "필라테스 후기",
        "일본 오사카", "태국 방콕", "베트남 다낭", "맥북 비교", "갤럭시 후기",
        "홍대 라멘", "연남동 브런치", "을지로 술집", "망원동 빵집", "삼청동 카페",
        "가로수길 맛집", "잠실 데이트", "여의도 맛집", "신촌 술집", "건대 카페",
    ],
    "rank": ["1위", "2위", "3위", "4위", "5위", "7위", "1페이지", "상위권", "탑3", "탑5"],
    "months": ["1", "2", "3", "4", "5", "6", "7", "8", "10", "12", "14", "18"],
    "days": ["일주일", "10일", "2주", "3주", "한달", "두달", "3개월", "5일"],
    "frequency": ["1", "2", "3", "매일 1", "이틀에 1", "주 3", "주 5"],
    "struggle": [
        "솔직히 포기하려 했는데", "중간에 쉬기도 했는데", "막막했는데",
        "안될줄 알았는데", "기대 안했는데", "이번에도 안될줄 알았는데",
        "3번이나 실패했었는데", "거의 접으려고 했는데", "슬럼프 심했는데",
        "한달간 노출 0이었는데", "계속 2페이지에서 안올라갔는데",
    ],
    "method": [
        "키워드 분석 열심히 했음", "글 퀄리티에 신경썼음", "꾸준히만 했음",
        "블랭크로 경쟁분석 했음", "사진 많이 넣었음", "2000자 이상 썼음",
        "소제목 구조 잡아서 썼음", "체험형 리뷰로 바꿨음", "내 경험 위주로 썼음",
        "검색 의도에 맞게 썼음", "관련 키워드 3개 추가했음", "서론 줄이고 핵심만 썼음",
        "직접 찍은 사진만 넣었음", "본문에 소제목 4개 넣었음",
    ],
    "method_name": [
        "매일 1포 쓰면서 키워드 3개 넣기", "제목에 핵심 키워드 넣고 본문 2천자 쓰기",
        "이웃 늘리면서 댓글 소통 열심히 하기", "경쟁 낮은 롱테일 키워드만 공략하기",
        "사진 15장 이상에 직촬만 사용하기", "네이버 인플루언서 지원하면서 글 쓰기",
        "시리즈물로 같은 주제 5편 이상 쓰기", "검색자 입장에서 질문형 제목 쓰기",
        "블랭크로 상위글 분석하고 구조 따라하기", "글 발행 후 30분 이내 서로이웃 방문",
        "트렌드 키워드 잡아서 당일 발행하기", "경쟁분석 먼저 하고 빈틈 공략하기",
    ],
    "stats": [
        "지수 35점 / 레벨 5", "C-Rank 15점 정도", "이웃 100명",
        "포스팅 50개", "일평균 방문자 30명", "이웃 200명 / 일방문 50",
        "C-Rank 25점 / 글 80개", "방문자 하루 100명 정도", "블로그 지수 40점대",
        "레벨 4 / 이웃 150명", "일 방문자 200명 / 이웃 300명",
        "글 120개 / 방문자 80명", "C-Rank 35점 / 레벨 6",
        "이웃 50명 / 글 30개", "블로그 지수 20점 초반",
    ],
    "doubt": [
        "진짜 효과 있을까 싶음", "너무 뻔한 얘기 아닌가", "개인차 있을것 같은데",
        "광고 아닌가 싶기도 하고", "검증된건지 모르겠음", "운이 좋았던거 아닐까",
        "그때랑 지금이랑 알고리즘 다르지 않나", "대형 블로거한테만 해당되는 얘기 같은데",
        "결국 돈 쓰라는 거 아님?", "실제로 해본사람 별로 없는것 같은데",
    ],
    "complaint": [
        "글 쓸 시간이 없음", "아이디어가 안떠오름", "매번 같은 얘기만 하게됨",
        "조회수가 안나옴", "상위노출이 안됨", "이웃이 안늘음",
        "분석만 하고 실행을 못하겠음", "주제 선정이 제일 어려움",
        "사진 찍는게 너무 귀찮음", "글감이 고갈됨",
        "비슷한 글 너무 많아서 차별화가 안됨", "알고리즘이 갑자기 바뀐것 같음",
    ],
    "topic": [
        "키워드 분석", "C-Rank", "상위노출", "블로그 지수", "이웃 늘리기",
        "포스팅 전략", "사진 촬영", "글 구조 잡기", "검색 최적화", "콘텐츠 기획",
        "블로그 수익화", "체험단 활용", "리뷰 작성법", "태그 활용법", "서로이웃 관리",
    ],
    "achievement": [
        "레벨7", "이웃 500명", "첫 상위노출", "월 방문자 1만", "첫 체험단",
        "이웃 1000명", "일 방문자 500명", "C-Rank 50점", "첫 원고료",
        "블로그 지수 60점", "연속 7일 상위노출", "월 수익 10만원",
    ],
    "next_goal": [
        "레벨9", "이웃 1000명", "1위 달성", "수익화", "인플루언서 신청",
        "월 50만원", "일 방문자 1000명", "C-Rank 70점", "전문 블로거 전환",
    ],
    "tip_content": [
        "키워드 검색할 때 네이버 자동완성을 꼭 활용하세요\n연관검색어 3~5개를 본문에 자연스럽게 녹여넣으면 노출범위가 넓어져요",
        "사진은 무조건 직접 찍은 거 쓰세요\n저작권 문제도 있고 네이버가 원본 이미지를 더 높게 평가한다고 하더라구요",
        "글 서론을 최대한 짧게 쓰세요\n검색해서 들어온 사람은 답을 빨리 알고 싶어해요\n쓸데없는 인사말 쓰면 이탈률만 올라감",
        "제목에 키워드 넣을 때 자연스럽게 넣으세요\n억지로 키워드 나열하면 오히려 역효과임",
        "같은 주제로 시리즈물 3~5편 쓰면 주제 전문성이 올라가요\n네이버가 해당 주제를 잘 아는 블로그로 인식함",
        "글 발행 후 1시간 이내가 중요해요\n그때 들어오는 트래픽이 초기 순위에 영향을 줌\n서로이웃한테 공유하세요",
        "본문에 소제목 3~4개 넣으세요\n가독성 높아지고 체류시간도 올라감\n네이버가 체류시간을 되게 중요하게 봄",
        "경쟁 키워드 바로 도전하지 말고\n관련된 세부 키워드부터 공략하세요\n거기서 전문성 쌓이면 큰 키워드도 먹힘",
        "블랭크에서 상위 10개 글 분석해보세요\n제목 패턴, 글자수, 이미지 수 파악하면\n뭘 따라하면 되는지 감이 와요",
        "댓글 답글 꼭꼭 달아주세요\n소통 지수가 블로그 지수에 포함되는듯\n이웃 방문도 자주 하시구요",
        "사진 alt 태그 꼭 넣으세요\n이미지 검색에서도 유입이 들어와요\n의외로 이거 안하는 분들 많더라",
        "글 끝에 관련 글 링크 3개 정도 걸어두세요\n내부링크가 많을수록 블로그 체류시간이 올라가요",
        "새벽이나 이른 아침에 발행하지 마세요\n사람들이 많이 검색하는 시간대에 맞춰 발행하면\n초기 유입이 확 다름",
        "리뷰 쓸 때 단점도 꼭 쓰세요\n장점만 나열하면 광고처럼 보여서\n오히려 신뢰도가 떨어져요",
        "제목 30자 이내로 쓰세요\n너무 길면 검색결과에서 잘리고\n클릭률이 떨어져요",
    ],
    "details": [
        "매일 아침 6시에 일어나서 글 썼음\n퇴근 후에는 너무 피곤해서 안되더라",
        "키워드 리서치만 3시간씩 했음\n좋은 키워드 찾으면 그날 바로 글 씀",
        "주말에 몰아서 5개씩 써놓고\n평일에 하나씩 발행했음",
        "처음 3개월은 진짜 아무 반응 없었음\n4개월째부터 서서히 올라오기 시작",
        "상위 블로그 글 50개 정도 분석했음\n공통점 파악하고 내 스타일로 변형함",
        "블랭크 분석 결과 보고 전략 수정했음\n약점 부분 집중 개선하니까 바로 효과 봄",
        "초반에 이웃 늘리기에만 집중했음\n이웃 300명 넘으니까 자연 유입도 생기더라",
        "글 하나당 평균 3시간 투자했음\n대충 쓰면 안된다는걸 깨달은 후로",
    ],
    "attempt_content": [
        "매일 글 2개씩 올리고 있는데\n방문자는 하루 20명도 안됨\n글 수만 늘린다고 되는게 아닌건지",
        "키워드 분석해서 경쟁 낮은거 골랐는데\n그래도 상위노출이 안됨\n내 지수가 너무 낮은건가",
        "사진 15장, 본문 3000자 이상 쓰는데\n비슷한 분량인 다른 블로그는 1등인데 나는 안보임",
        "이웃 500명인데 실제 방문하는 사람은 10명도 안됨\n이웃 수가 의미가 있긴 한건지",
        "3개월째 같은 키워드로 도전중인데\n처음엔 7페이지였다가 지금 4페이지\n근데 1페이지를 못 뚫겠음",
    ],
    "skeptical_points": [
        "상위노출 됐다는 사람들\n진짜 그 키워드가 검색량이 있긴 한건지\n경쟁 0인 키워드로 1등한거 아님?",
        "이웃 1000명 자랑하는데\n서로이웃 아니면 의미없지 않나\n숫자만 많으면 뭐하나",
        "월 수익 100만원이라는 사람들\n체험단 물건값 포함인거 아닌가\n순수 현금 수익이 궁금",
        "매일 1포 하라는데\n퀄리티 낮은 글 매일 올리는거랑\n좋은 글 주 3개 올리는거랑 뭐가 나음?",
        "블로그 지수 개념 자체가 의심됨\n네이버가 공식적으로 발표한 적 없는데\n다들 추측으로 얘기하는거 아님?",
    ],
    "tired_content": [
        "3개월째 매일 글 쓰는데\n조회수 변화가 없으니까 의욕이 안남\n이걸 왜 하고있나 싶음",
        "주변에서는 블로그 그만하라고 하는데\n나름 애착이 있어서 못 놓겠고\n근데 성과가 안나니까 힘듦",
        "글감 고갈이 심각함\n매일 뭘 써야할지 모르겠고\n억지로 쓰면 퀄리티도 안나옴",
        "다른 블로거들 보면 쉽게 하는것 같은데\n나만 이렇게 힘든건지\n뭘 잘못하고 있는건지도 모르겠음",
        "블로그 앱 켜기가 싫어짐\n예전에는 재밌었는데\n숫자에 집착하다보니 재미를 잃은듯",
    ],
}


def fill_template(template: str, user: VirtualUser) -> str:
    """템플릿에 변수 채우기"""
    result = template

    for key, values in TEMPLATE_DATA.items():
        placeholder = "{" + key + "}"
        while placeholder in result:
            result = result.replace(placeholder, random.choice(values), 1)

    # 사용자 정보로 대체
    result = result.replace("{user_months}", str(user.blog_months))

    return result


# ============ 제목 변형 시스템 (고정 제목 중복 방지) ============

TITLE_SUFFIXES_EMOTION = [
    "ㅠㅠ", "ㅜㅜ", "ㅋㅋ", "ㅋㅋㅋ", "ㅎㅎ", "ㅎ", "..", "...", ";;;",
    "ㄷㄷ", "ㅠ", "ㅜ", "ㅋ", "!!", "!", "~", "ㅎㅎㅎ",
]

TITLE_SUFFIXES_PERIOD = [
    "(1개월차)", "(2개월차)", "(3개월차)", "(4개월차)", "(5개월차)",
    "(6개월차)", "(8개월차)", "(10개월차)", "(1년차)", "(초보)",
    "(입문)", "(중급)", "(n개월차)",
]

TITLE_SUFFIXES_REACTION = [
    "진짜", "아", "후", "하", "흠", "으으", "아아",
    "ㄹㅇ", "솔직히", "대충", "그냥",
]

TITLE_SUFFIXES_TAG = [
    "(블로그)", "(일상)", "(고민)", "(잡담)", "(경험담)",
    "(후기)", "(현실)", "(TMI)", "(공감)", "(토크)",
]

ALL_TITLE_SUFFIXES = (
    TITLE_SUFFIXES_EMOTION
    + TITLE_SUFFIXES_PERIOD
    + TITLE_SUFFIXES_REACTION
    + TITLE_SUFFIXES_TAG
)


def _add_title_variation(title: str) -> str:
    """고정 제목(placeholder 없는 제목)에 자연스러운 변형 접미사 추가.

    변형 패턴:
    - 감정 표현: "슬럼프 온듯" → "슬럼프 온듯 ㅠㅠ"
    - 기간 표현: "블로그 하기 싫다" → "블로그 하기 싫다 (3개월차)"
    - 감탄 표현: "번아웃 왔어요" → "번아웃 왔어요 진짜"
    """
    suffix = random.choice(ALL_TITLE_SUFFIXES)
    # 이미 비슷한 접미사가 있으면 다른 카테고리에서 선택
    if suffix.strip("()") in title:
        pool = random.choice([
            TITLE_SUFFIXES_EMOTION,
            TITLE_SUFFIXES_PERIOD,
            TITLE_SUFFIXES_REACTION,
            TITLE_SUFFIXES_TAG,
        ])
        suffix = random.choice(pool)
    # 공백 + 접미사
    return f"{title} {suffix}"


# ============ 맥락 인식 댓글 생성 ============

def generate_contextual_comment(post: Dict, commenter: VirtualUser) -> str:
    """게시글 맥락에 맞는 댓글 생성"""
    post_mood = post.get("mood", Mood.CURIOUS)
    post_category = post.get("category", "free")
    post_content = post.get("content", "")

    # 댓글 무드 결정 (게시글 무드와 댓글러 성격에 따라)
    if commenter.personality == Personality.SARCASTIC:
        comment_mood = random.choice([Mood.SKEPTICAL, Mood.SARCASTIC])
    elif commenter.personality == Personality.FRIENDLY:
        comment_mood = random.choice([Mood.HAPPY, Mood.HELPFUL])
    elif post_mood in [Mood.FRUSTRATED, Mood.TIRED]:
        comment_mood = random.choice([Mood.FRUSTRATED, Mood.HELPFUL])
    else:
        comment_mood = random.choice([Mood.HAPPY, Mood.CURIOUS, Mood.HELPFUL])

    # 기본 템플릿에서 선택
    base_comments = COMMENT_TEMPLATES_BY_MOOD.get(comment_mood, COMMENT_TEMPLATES_BY_MOOD[Mood.HAPPY])
    comment = random.choice(base_comments)

    # 게시글 내용에 따른 추가 (키워드 언급)
    if "상위노출" in post_content or "성공" in post.get("title", ""):
        extras = [
            " 어떤 키워드예요?", " 얼마나 걸렸어요?", " 비결이 뭐예요?",
            " 저도 도전해볼게요!", " 부럽다 ㅠㅠ",
        ]
        if random.random() < 0.4:
            comment += random.choice(extras)

    elif "질문" in post_category or "?" in post.get("title", ""):
        if commenter.blog_months > 6:  # 경력자는 조언
            advice = [
                " 저는 이렇게 했어요~", " 제 경험상은요...", " 꾸준히 하시면 돼요!",
            ]
            if random.random() < 0.3:
                comment += random.choice(advice)

    # 성격에 따른 후처리
    transformer = NaturalTextTransformer()

    if commenter.writing_style.get("uses_slang"):
        comment = transformer.apply_slang(comment, 0.4)

    # 이모티콘 추가
    if commenter.writing_style.get("uses_emoticons") and random.random() < 0.5:
        if comment_mood in [Mood.HAPPY, Mood.HELPFUL]:
            comment += " " + random.choice(EMOTICONS["positive"])
        elif comment_mood in [Mood.FRUSTRATED, Mood.TIRED]:
            comment += " " + random.choice(EMOTICONS["negative"])

    # 끝에 ㅋㅋ, ㅎㅎ 추가 (캐주얼한 성격)
    if commenter.personality == Personality.CASUAL and random.random() < 0.3:
        comment += random.choice(["ㅋㅋ", "ㅎㅎ", "ㅋㅋㅋ"])

    return comment


# ============ 대댓글 생성 (대화 흐름) ============

REPLY_PATTERNS = {
    "answer_question": [
        "앗 {keyword}요!", "{days} 정도 걸렸어요", "저는 {method} 했어요",
        "블랭크에서 분석해보세요!", "꾸준히 하다보면 돼요",
        "아 그건 {method} 해보세요", "저는 {months}개월쯤부터 효과봤어요",
        "한번 해보시고 결과 알려주세요!", "다른 분들도 비슷하게 물어보시더라",
    ],
    "thank_reply": [
        "감사합니다!", "ㄳㄳ", "도움됐어요!", "참고할게요~", "화이팅이에요!",
        "아 감사해요 바로 해볼게요", "오 그렇군요!", "좋은 정보네요 감사!",
    ],
    "agree": [
        "맞아요 ㅋㅋ", "ㅇㅈ", "그쵸", "저도요!", "공감합니다",
        "진짜 그래요", "완전 공감", "저도 같은 생각이에요", "이거 맞는말임",
    ],
    "disagree": [
        "음 저는 좀 달랐는데", "사바사일듯", "글쎄요~", "저는 반대였어요",
        "저는 다르게 했는데 효과봤어요", "그게 꼭 맞지는 않은것 같아요",
        "케이스가 다를수도 있지 않나요", "저는 그 방법 안맞았어요",
    ],
}


def generate_reply(parent_comment: str, original_poster: VirtualUser, replier: VirtualUser) -> Optional[str]:
    """대댓글 생성"""
    # 원글쓴이가 답글 다는 경우
    if replier.id == original_poster.id:
        if "?" in parent_comment or "어떻게" in parent_comment or "뭐" in parent_comment:
            reply = random.choice(REPLY_PATTERNS["answer_question"])
            reply = fill_template(reply, original_poster)
        else:
            reply = random.choice(REPLY_PATTERNS["thank_reply"])
    else:
        # 다른 사람이 대댓글
        if random.random() < 0.5:
            reply = random.choice(REPLY_PATTERNS["agree"])
        else:
            reply = random.choice(REPLY_PATTERNS["disagree"])

    # 스타일 적용
    if replier.writing_style.get("uses_emoticons") and random.random() < 0.3:
        reply += " " + random.choice(EMOTICONS["neutral"])

    return reply


# ============ 게시글 생성 ============

def generate_natural_post(user: VirtualUser = None, category: str = None) -> Dict:
    """자연스러운 게시글 생성"""
    if user is None:
        user = generate_virtual_user()

    # 카테고리 결정 (없으면 랜덤)
    if category is None:
        categories = ["free", "tip", "question", "success"]
        weights = [35, 25, 25, 15]
        category = random.choices(categories, weights=weights)[0]

    # 무드 결정 (사용자 상태, 카테고리에 따라)
    if category == "success":
        mood = random.choice([Mood.HAPPY, Mood.BRAGGING, Mood.EXCITED])
    elif category == "question":
        mood = random.choice([Mood.CURIOUS, Mood.FRUSTRATED, Mood.SKEPTICAL])
    elif category == "tip":
        mood = random.choice([Mood.HELPFUL, Mood.HAPPY])
    else:
        mood = random.choice(list(Mood))

    # 템플릿 선택
    mood_templates = POST_TEMPLATES_BY_MOOD.get(mood, {})
    category_templates = mood_templates.get(category, [])

    if not category_templates:
        # 폴백: 기본 템플릿
        category_templates = POST_TEMPLATES_BY_MOOD[Mood.CURIOUS].get("question", [
            {"title": "블로그 질문이요", "content": "질문 있는데요..."}
        ])

    template = random.choice(category_templates)

    # 템플릿 채우기
    title = fill_template(template["title"], user)
    content = fill_template(template["content"], user)

    # 고정 제목(placeholder가 없었던 제목)에 변형 적용
    if "{" not in template["title"]:
        title = _add_title_variation(title)

    # 자연스러운 변환 적용
    transformer = NaturalTextTransformer()

    if user.writing_style.get("uses_slang"):
        content = transformer.apply_slang(content, 0.3)

    content = transformer.add_fillers(content, user)
    content = transformer.add_emoticons(content, user, mood)

    # 가끔 오타 추가 (5% 확률)
    if random.random() < 0.05:
        content = transformer.add_typos(content, 0.01)

    # 비속어 추가 (특정 무드 + 성격)
    if mood in [Mood.FRUSTRATED, Mood.COMPLAINING] and user.writing_style.get("uses_expletives"):
        if random.random() < user.writing_style.get("expletive_frequency", 0.05):
            expletive = random.choice(MILD_EXPLETIVES)
            content = f"{expletive} {content}"

    return {
        "user_id": user.id,
        "user_name": user.name,
        "user_level": user.skill_level,
        "title": title,
        "content": content,
        "category": category,
        "mood": mood,
        "tags": [],
    }


# ============ 메인 생성 함수 ============

def generate_daily_natural_content(
    posts_count: Tuple[int, int] = (15, 30),
    comments_per_post: Tuple[int, int] = (3, 12),
) -> Dict:
    """하루치 자연스러운 콘텐츠 생성 (중복 방지 포함)"""
    from database.community_db import get_db_connection, is_title_duplicate
    import json

    conn = get_db_connection()
    cursor = conn.cursor()

    # 오늘 생성할 게시글 수 (범위 내 랜덤)
    num_posts = random.randint(*posts_count)

    # 시간 분포 (자연스럽게)
    now = datetime.now()

    created_posts = []
    created_comments = []
    skipped_duplicates = 0

    # ★ 배치 내 중복 방지용 세트
    used_titles_in_batch: set = set()

    for i in range(num_posts):
        # 랜덤 시간 (오늘 중)
        hour = random.choices(
            range(24),
            weights=[1,1,1,1,1,2,3,4,5,6,7,7,6,6,7,7,7,6,7,8,9,8,6,3]
        )[0]
        minute = random.randint(0, 59)
        post_time = now.replace(hour=hour, minute=minute, second=random.randint(0, 59))

        if post_time > now:
            post_time = now - timedelta(minutes=random.randint(1, 60))

        # 게시글 생성 (최대 3회 재시도)
        user = generate_virtual_user()
        post = None
        for _attempt in range(3):
            candidate = generate_natural_post(user)
            title = candidate["title"]

            # (1) 배치 내 중복 체크
            if title in used_titles_in_batch:
                continue

            # (2) DB 중복 체크 (72시간)
            if is_title_duplicate(title, hours=72):
                continue

            post = candidate
            break

        if post is None:
            skipped_duplicates += 1
            continue

        used_titles_in_batch.add(post["title"])

        # DB 삽입
        cursor.execute("""
            INSERT INTO posts (user_id, user_name, title, content, category, tags, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (
            post["user_id"],
            post["user_name"],
            post["title"],
            post["content"],
            post["category"],
            json.dumps(post.get("tags", [])),
            post_time.isoformat()
        ))

        post_id = cursor.lastrowid
        post["id"] = post_id
        post["created_at"] = post_time
        created_posts.append(post)

        # 댓글 생성
        num_comments = random.randint(*comments_per_post)

        for j in range(num_comments):
            commenter = generate_virtual_user()
            comment_text = generate_contextual_comment(post, commenter)

            # 댓글 시간 (게시글 이후)
            comment_time = post_time + timedelta(
                minutes=random.randint(5, 60 * 24)  # 5분 ~ 24시간 후
            )
            if comment_time > now:
                comment_time = now - timedelta(minutes=random.randint(1, 30))

            cursor.execute("""
                INSERT INTO post_comments (post_id, user_id, user_name, content, created_at)
                VALUES (?, ?, ?, ?, ?)
            """, (
                post_id,
                commenter.id,
                commenter.name,
                comment_text,
                comment_time.isoformat()
            ))

            comment_id = cursor.lastrowid
            created_comments.append({
                "id": comment_id,
                "post_id": post_id,
                "content": comment_text,
            })

            # 대댓글 (30% 확률)
            if random.random() < 0.3:
                reply = generate_reply(comment_text, user, user)  # 원글쓴이 답글
                if reply:
                    reply_time = comment_time + timedelta(minutes=random.randint(5, 120))
                    if reply_time > now:
                        reply_time = now - timedelta(minutes=random.randint(1, 10))

                    cursor.execute("""
                        INSERT INTO post_comments (post_id, user_id, user_name, content, parent_id, created_at)
                        VALUES (?, ?, ?, ?, ?, ?)
                    """, (
                        post_id,
                        user.id,
                        user.name,
                        reply,
                        comment_id,
                        reply_time.isoformat()
                    ))
                    created_comments.append({
                        "id": cursor.lastrowid,
                        "post_id": post_id,
                        "parent_id": comment_id,
                        "content": reply,
                    })

        # 댓글 수 업데이트
        cursor.execute("""
            UPDATE posts SET comments_count = (
                SELECT COUNT(*) FROM post_comments WHERE post_id = ? AND is_deleted = FALSE
            ) WHERE id = ?
        """, (post_id, post_id))

    conn.commit()
    conn.close()

    logger.info(
        f"Generated {len(created_posts)} posts and {len(created_comments)} comments "
        f"(skipped {skipped_duplicates} duplicates)"
    )

    return {
        "posts_created": len(created_posts),
        "comments_created": len(created_comments),
        "duplicates_skipped": skipped_duplicates,
        "timestamp": now.isoformat(),
    }


# ============ 기존 게시글에 댓글 추가 ============

def add_comments_to_existing_posts(
    max_posts: int = 20,
    comments_per_post: Tuple[int, int] = (1, 5),
) -> Dict:
    """기존 게시글에 새 댓글 추가"""
    from database.community_db import get_db_connection

    conn = get_db_connection()
    cursor = conn.cursor()

    # 최근 게시글 조회 (댓글 적은 것 우선)
    cursor.execute("""
        SELECT id, user_id, user_name, title, content, category
        FROM posts
        WHERE is_deleted = FALSE
        ORDER BY comments_count ASC, created_at DESC
        LIMIT ?
    """, (max_posts,))

    posts = cursor.fetchall()

    now = datetime.now()
    total_comments = 0

    for post in posts:
        post_dict = {
            "id": post["id"],
            "user_id": post["user_id"],
            "title": post["title"],
            "content": post["content"],
            "category": post["category"],
        }

        original_user = generate_virtual_user(post["user_id"])
        num_comments = random.randint(*comments_per_post)

        for _ in range(num_comments):
            commenter = generate_virtual_user()
            comment_text = generate_contextual_comment(post_dict, commenter)

            comment_time = now - timedelta(minutes=random.randint(1, 60 * 6))

            cursor.execute("""
                INSERT INTO post_comments (post_id, user_id, user_name, content, created_at)
                VALUES (?, ?, ?, ?, ?)
            """, (
                post["id"],
                commenter.id,
                commenter.name,
                comment_text,
                comment_time.isoformat()
            ))

            total_comments += 1

        # 댓글 수 업데이트
        cursor.execute("""
            UPDATE posts SET comments_count = (
                SELECT COUNT(*) FROM post_comments WHERE post_id = ? AND is_deleted = FALSE
            ) WHERE id = ?
        """, (post["id"], post["id"]))

    conn.commit()
    conn.close()

    return {
        "posts_updated": len(posts),
        "comments_added": total_comments,
    }
