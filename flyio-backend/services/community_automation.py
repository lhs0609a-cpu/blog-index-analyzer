"""
커뮤니티 자동화 시스템
- 가상 사용자로 게시글/댓글 자동 생성
- 커뮤니티 활성화 효과
"""
import random
import asyncio
from datetime import datetime, timedelta
from typing import List, Dict
import logging

logger = logging.getLogger(__name__)

# ============ 가상 블로거 이름 풀 ============
BLOGGER_NAMES = [
    # 귀여운 닉네임
    "꿈꾸는블로거", "행복한하루", "소소한일상", "달콤한인생", "빛나는오늘",
    "설레는마음", "따뜻한커피", "포근한오후", "싱그러운아침", "반짝이는별",

    # 블로그 관련
    "블로그마스터", "상위노출러", "키워드헌터", "SEO달인", "블로그초보탈출",
    "네이버정복자", "블링크매니아", "검색왕도전", "트래픽폭발러", "조회수부자",

    # 직업/관심사
    "마케터김대리", "디자이너윤씨", "개발자박씨", "프리랜서생활", "N잡러일상",
    "육아맘블로그", "워킹맘다이어리", "직장인브이로그", "대학생일기", "취준생화이팅",

    # 지역 기반
    "강남블로거", "부산사람", "제주도민", "인천토박이", "대전살이",
    "경기도민", "서울러버", "광주사랑", "대구생활", "울산일상",

    # 감성 닉네임
    "하늘바라기", "바다향기", "숲속여행자", "별빛산책", "구름위산책",
    "햇살가득", "달빛아래", "새벽감성", "노을빛하늘", "봄날의햇살",

    # 동물/캐릭터
    "귀여운고양이", "멍멍이집사", "토끼사랑", "펭귄워들", "곰돌이푸",
    "다람쥐점프", "부엉이야행", "나비날개", "꿀벌윙윙", "여우꼬리",

    # 음식/취미
    "카페투어러", "맛집헌터", "디저트러버", "커피중독자", "빵순이",
    "요리초보", "베이킹마니아", "와인한잔", "치맥사랑", "라면매니아",

    # 운동/건강
    "헬스초보", "러닝맨", "요가하는날", "필라테스러버", "등산매니아",
    "자전거타는날", "수영사랑", "홈트일기", "다이어트도전", "건강한삶",

    # 기타
    "열정가득", "도전정신", "긍정에너지", "성장일기", "배움의길",
    "꾸준함이답", "매일성장", "오늘도화이팅", "내일은더좋아", "할수있다",
]

# ============ 게시글 템플릿 ============
POST_TEMPLATES = {
    "tip": [
        {
            "title": "블로그 상위노출 성공한 방법 공유합니다!",
            "content": """안녕하세요! 블로그 시작한 지 3개월 된 초보입니다.

드디어 첫 상위노출에 성공했어요! 🎉

제가 해본 방법 공유할게요:

1. **키워드 선정이 진짜 중요해요**
   - 블랭크에서 경쟁 분석 먼저 했어요
   - 상위 블로그들 지수 확인하고 내 수준에 맞는 키워드 선택

2. **글 분량은 2000자 이상**
   - 너무 짧으면 안 되더라구요
   - 사진도 10장 이상 넣었어요

3. **제목에 키워드 꼭 넣기**
   - 자연스럽게 넣는 게 포인트!

혹시 궁금한 거 있으시면 댓글 남겨주세요~"""
        },
        {
            "title": "C-Rank 올리는 현실적인 방법 (경험담)",
            "content": """블로그 1년차인데 C-Rank가 안 올라서 고민 많았어요.

근데 최근에 확 올랐거든요! 어떻게 했는지 공유합니다.

**1. 이웃 관리**
- 진짜 소통하는 이웃 늘리기
- 서로이웃 신청 적극적으로

**2. 댓글 활동**
- 다른 블로그 방문해서 진심 담은 댓글 달기
- 하루에 10개 이상은 했어요

**3. 꾸준한 포스팅**
- 일주일에 최소 3개
- 주제 일관성 있게!

**4. 블랭크로 내 지수 체크**
- 매주 분석해서 어디가 부족한지 확인
- D.I.A. 점수 특히 신경썼어요

한 달 정도 하니까 효과 보이더라구요.
다들 화이팅! 💪"""
        },
        {
            "title": "키워드 분석 이렇게 하니까 상위노출 됨",
            "content": """오늘 드디어 목표했던 키워드 1페이지 진입했어요!!

블랭크 키워드 분석 기능 진짜 유용하더라구요.

**제가 한 방법:**

1. 먼저 메인 키워드로 검색
2. 상위 10개 블로그 지수 확인
3. 내 블로그 지수랑 비교
4. 비슷하거나 낮은 키워드 선택
5. 연관 키워드도 함께 확인

처음엔 경쟁 심한 키워드만 노렸는데
그러니까 안 되더라구요 ㅠㅠ

눈높이 낮추고 할 만한 키워드부터 공략하니까
하나씩 되기 시작했어요!

다들 너무 경쟁 심한 키워드만 노리지 마세요~"""
        },
    ],
    "question": [
        {
            "title": "블로그 지수 어느 정도면 괜찮은 건가요?",
            "content": """블로그 시작한 지 2개월 됐는데요.

블랭크로 분석해보니까
- 총점: 32점
- 레벨: Lv.5
- C-Rank: 12점
- D.I.A.: 18점

이 정도면 어느 정도 수준인가요?
아직 상위노출은 한 번도 못 해봤어요 ㅠㅠ

선배님들 조언 부탁드립니다!"""
        },
        {
            "title": "하루에 포스팅 몇 개가 적당할까요?",
            "content": """요즘 블로그 열심히 하려고 하는데
하루에 몇 개 정도 올리는 게 좋을까요?

너무 많이 올리면 저품질 걸린다는 말도 있고
꾸준히 매일 올려야 한다는 말도 있고...

혼란스럽네요 ㅠㅠ

고수님들은 어떻게 하시나요?"""
        },
        {
            "title": "이웃 수가 적은데 상위노출 가능할까요?",
            "content": """블로그 이웃이 50명밖에 안 되는데
상위노출 할 수 있을까요?

C-Rank가 이웃 수랑 관련 있다고 해서 걱정이에요.

이웃 적어도 상위노출 성공하신 분 계신가요?
어떻게 하셨는지 궁금합니다!"""
        },
    ],
    "success": [
        {
            "title": "🎉 첫 상위노출 성공! 너무 기뻐요",
            "content": """드디어!!!! 첫 상위노출 성공했어요!!!!

'강남 맛집 추천' 키워드로 7위 찍었습니다 ㅠㅠ

블로그 시작한 지 4개월만이에요.

중간에 포기하고 싶을 때도 많았는데
블랭크로 꾸준히 분석하면서 부족한 점 개선했더니
드디어 되네요!!

다들 포기하지 마세요!
저도 했으니까 여러분도 할 수 있어요! 💪"""
        },
        {
            "title": "레벨 7 달성! 성장 과정 공유",
            "content": """오늘 드디어 레벨 7 달성했습니다!

**성장 과정:**
- 1개월차: Lv.3 (막막했음)
- 2개월차: Lv.4 (조금씩 감 잡음)
- 3개월차: Lv.5 (첫 상위노출)
- 4개월차: Lv.6 (꾸준히 성장)
- 5개월차: Lv.7 (목표 달성!)

**핵심은 꾸준함이었어요:**
- 매일 1포스팅
- 매주 블랭크로 지수 체크
- 부족한 점 계속 개선

다음 목표는 Lv.9입니다!
다들 화이팅! 🔥"""
        },
    ],
    "review": [
        {
            "title": "블랭크 한 달 사용 후기 (솔직 리뷰)",
            "content": """블랭크 한 달 써본 솔직한 후기입니다.

**좋았던 점:**
- 내 블로그 상태를 객관적으로 알 수 있음
- 키워드 경쟁 분석이 진짜 유용함
- 상위 블로그들과 비교해서 뭐가 부족한지 앎
- UI가 깔끔하고 쓰기 편함

**아쉬운 점:**
- 분석 횟수 제한 (근데 이해는 함)
- 가끔 로딩이 느릴 때 있음

**결론:**
블로그 진지하게 하시는 분들한텐 추천이요!
특히 키워드 분석은 진짜 도움 많이 됐어요.

무료로도 충분히 쓸만합니다 👍"""
        },
    ],
    "free": [
        {
            "title": "오늘도 블로그 화이팅!",
            "content": """오늘도 열심히 포스팅 했습니다!

비 오는 날이라 카페에서 작업했는데
분위기 좋아서 집중 잘 되네요 ☕

다들 오늘 하루도 수고하셨어요!
내일도 화이팅! 💪"""
        },
        {
            "title": "블로그 슬럼프 왔는데 어떡하죠 ㅠㅠ",
            "content": """요즘 블로그 글 쓰기가 너무 힘들어요.

뭘 써야 할지도 모르겠고
쓰고 싶은 의욕도 안 나고...

다들 슬럼프 올 때 어떻게 극복하시나요?
조언 좀 부탁드려요 ㅠㅠ"""
        },
        {
            "title": "블로그하면서 생긴 좋은 습관",
            "content": """블로그 1년 하면서 생긴 좋은 습관들이 있어요.

1. **기록하는 습관**
   - 일상을 더 관찰하게 됨
   - 사진 찍는 습관

2. **글쓰기 능력 향상**
   - 처음엔 1000자도 힘들었는데
   - 지금은 3000자도 금방 씀

3. **꾸준함**
   - 매일 조금씩이라도 하는 습관
   - 이게 다른 곳에도 적용됨

블로그 시작하길 잘했다는 생각이 들어요!
다들 화이팅입니다 😊"""
        },
    ],
}

# ============ 댓글 템플릿 ============
COMMENT_TEMPLATES = [
    # 공감/응원
    "저도 공감해요! 화이팅입니다 💪",
    "좋은 정보 감사합니다!",
    "오 이거 진짜 도움 되네요!",
    "저도 이렇게 해봐야겠어요~",
    "대박 꿀팁이에요!!",
    "와 진짜요? 저도 해볼게요!",
    "응원합니다! 화이팅! 🔥",
    "저도 같은 고민이었는데 도움 됐어요",
    "오늘도 좋은 글 감사해요~",
    "블로그 같이 성장해요! 💪",

    # 질문/대화
    "혹시 얼마나 걸리셨어요?",
    "저도 시도해볼게요! 감사합니다",
    "오 이 방법 처음 알았어요!",
    "구체적인 팁 감사합니다~",
    "저도 비슷한 경험 있어요!",
    "진짜 도움 많이 되는 글이네요",
    "북마크 해둘게요!",
    "나중에 참고할게요 감사합니다",

    # 축하
    "축하드려요!! 🎉",
    "와 대단하세요!",
    "멋져요! 저도 열심히 할게요",
    "부럽습니다 ㅠㅠ 저도 열심히 해야겠어요",
    "역시 꾸준함이 답이네요!",

    # 공유
    "저는 이렇게 했는데 효과 봤어요~",
    "비슷한 경험 있어서 공감되네요",
    "저도 블랭크 쓰는데 좋더라구요!",
    "맞아요 키워드 분석 진짜 중요해요",

    # 격려
    "포기하지 마세요! 분명 될 거예요",
    "저도 처음엔 그랬어요. 화이팅!",
    "꾸준히 하시면 분명 좋아질 거예요",
    "다들 처음엔 힘들어요. 같이 힘내요!",
    "응원할게요! 화이팅입니다 💪",
]


def get_random_blogger_name() -> str:
    """랜덤 블로거 이름 생성"""
    return random.choice(BLOGGER_NAMES)


def get_random_post(category: str = None) -> Dict:
    """랜덤 게시글 생성"""
    if category and category in POST_TEMPLATES:
        templates = POST_TEMPLATES[category]
    else:
        # 전체 카테고리에서 랜덤 선택
        all_templates = []
        for cat_templates in POST_TEMPLATES.values():
            all_templates.extend([(cat, t) for t in cat_templates for cat in POST_TEMPLATES if t in POST_TEMPLATES[cat]])

        category = random.choice(list(POST_TEMPLATES.keys()))
        templates = POST_TEMPLATES[category]

    template = random.choice(templates)

    return {
        "title": template["title"],
        "content": template["content"],
        "category": category,
        "author_name": get_random_blogger_name()
    }


def get_random_comment() -> str:
    """랜덤 댓글 생성"""
    return random.choice(COMMENT_TEMPLATES)


# ============ 자동 생성 함수 ============

def generate_seed_posts(count: int = 20) -> List[int]:
    """초기 시드 게시글 생성"""
    from database.community_db import get_db_connection
    import json

    conn = get_db_connection()
    cursor = conn.cursor()

    created_ids = []

    for i in range(count):
        post = get_random_post()

        # 가상 사용자 ID (10000 이상으로 실제 사용자와 구분)
        fake_user_id = random.randint(10000, 99999)

        # 랜덤 시간 (최근 7일 내)
        days_ago = random.randint(0, 7)
        hours_ago = random.randint(0, 23)
        created_at = (datetime.now() - timedelta(days=days_ago, hours=hours_ago)).isoformat()

        # 랜덤 통계
        views = random.randint(10, 500)
        likes = random.randint(0, min(views // 5, 50))

        cursor.execute("""
            INSERT INTO posts (user_id, user_name, title, content, category, views, likes, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (fake_user_id, post["author_name"], post["title"], post["content"],
              post["category"], views, likes, created_at))

        created_ids.append(cursor.lastrowid)

    conn.commit()
    conn.close()

    logger.info(f"Generated {len(created_ids)} seed posts")
    return created_ids


def generate_seed_comments(post_ids: List[int] = None, comments_per_post: tuple = (2, 8)) -> int:
    """초기 시드 댓글 생성"""
    from database.community_db import get_db_connection

    conn = get_db_connection()
    cursor = conn.cursor()

    # post_ids가 없으면 최근 게시글 조회
    if not post_ids:
        cursor.execute("SELECT id FROM posts ORDER BY created_at DESC LIMIT 50")
        post_ids = [row['id'] for row in cursor.fetchall()]

    total_comments = 0

    for post_id in post_ids:
        # 게시글당 랜덤 댓글 수
        num_comments = random.randint(comments_per_post[0], comments_per_post[1])

        for _ in range(num_comments):
            fake_user_id = random.randint(10000, 99999)
            author_name = get_random_blogger_name()
            comment = get_random_comment()

            # 랜덤 시간
            hours_ago = random.randint(0, 72)
            created_at = (datetime.now() - timedelta(hours=hours_ago)).isoformat()

            cursor.execute("""
                INSERT INTO post_comments (post_id, user_id, user_name, content, created_at)
                VALUES (?, ?, ?, ?, ?)
            """, (post_id, fake_user_id, author_name, comment, created_at))

            total_comments += 1

        # 댓글 수 업데이트
        cursor.execute("""
            UPDATE posts SET comments_count = (
                SELECT COUNT(*) FROM post_comments WHERE post_id = ? AND is_deleted = FALSE
            ) WHERE id = ?
        """, (post_id, post_id))

    conn.commit()
    conn.close()

    logger.info(f"Generated {total_comments} seed comments")
    return total_comments


def generate_daily_content() -> Dict:
    """매일 자동 콘텐츠 생성 (스케줄러에서 호출)"""
    from database.community_db import get_db_connection

    conn = get_db_connection()
    cursor = conn.cursor()

    result = {
        "posts_created": 0,
        "comments_created": 0
    }

    # 1. 새 게시글 생성 (3~5개)
    num_posts = random.randint(3, 5)
    new_post_ids = []

    for _ in range(num_posts):
        post = get_random_post()
        fake_user_id = random.randint(10000, 99999)

        # 오늘 중 랜덤 시간
        hours_ago = random.randint(0, 12)
        created_at = (datetime.now() - timedelta(hours=hours_ago)).isoformat()

        cursor.execute("""
            INSERT INTO posts (user_id, user_name, title, content, category, created_at)
            VALUES (?, ?, ?, ?, ?, ?)
        """, (fake_user_id, post["author_name"], post["title"], post["content"],
              post["category"], created_at))

        new_post_ids.append(cursor.lastrowid)
        result["posts_created"] += 1

    conn.commit()

    # 2. 기존 게시글에 댓글 추가 (10~20개)
    cursor.execute("SELECT id FROM posts WHERE is_deleted = FALSE ORDER BY created_at DESC LIMIT 20")
    recent_posts = [row['id'] for row in cursor.fetchall()]

    num_comments = random.randint(10, 20)
    for _ in range(num_comments):
        post_id = random.choice(recent_posts) if recent_posts else None
        if not post_id:
            continue

        fake_user_id = random.randint(10000, 99999)
        author_name = get_random_blogger_name()
        comment = get_random_comment()

        cursor.execute("""
            INSERT INTO post_comments (post_id, user_id, user_name, content)
            VALUES (?, ?, ?, ?)
        """, (post_id, fake_user_id, author_name, comment))

        cursor.execute("""
            UPDATE posts SET comments_count = comments_count + 1 WHERE id = ?
        """, (post_id,))

        result["comments_created"] += 1

    conn.commit()
    conn.close()

    logger.info(f"Daily content generated: {result}")
    return result


def initialize_community() -> Dict:
    """커뮤니티 초기화 (시드 데이터 생성)"""
    logger.info("Initializing community with seed data...")

    # 1. 시드 게시글 생성 (30개)
    post_ids = generate_seed_posts(30)

    # 2. 시드 댓글 생성
    comments_count = generate_seed_comments(post_ids, (3, 10))

    return {
        "posts_created": len(post_ids),
        "comments_created": comments_count,
        "message": "Community initialized successfully"
    }
