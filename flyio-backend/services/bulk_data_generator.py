...
# 확장된 게시글 템플릿 (변수 치환 가능) - 자연스러운 한국어 표현
EXTENDED_POST_TEMPLATES = {
    "tip": [
        {
            "title": "'{keyword}' 상위노출 성공함 ㄹㅇ 개꿀팁",
            "content": """아 드디어 '{keyword}' 상위노출 됐다ㅋㅋㅋㅋ

{struggle} 됐음 ㅠㅠ 진짜 {emotion}
...
궁금한거 있으면 댓글 ㄱㄱ""",
        ...
        {
            "title": "C-Rank 올리는 법 내가 직접 해봄",
            "content": """C-Rank 안올라서 빡치는 사람 나만 있는거 아니지?ㅋㅋ

저도 {days} 동안 안올라서 미칠뻔 했는데 최근에 확 올랐음

효과 있었던거 정리함

1. 이웃 관리
서이추 열심히 함 ({neighbor_count} 목표)
진짜 소통하는 이웃 늘림
...
다들 화이팅임""",
        ...
        {
            "title": "글쓰기 시간 단축하는 꿀팁 공유",
            "content": """글 하나 쓰는데 {time_spent} 넘게 걸리는 사람?

나도 처음엔 그랬는데 지금은 {time_spent} 안에 끝남ㅋㅋ
...
처음엔 힘들어도 습관되면 금방임""",
        ...
        {
            "title": "블랭크 꿀팁 정리해봄",
            "content": """블랭크 쓰는 사람들 이거 봐

내가 발견한 꿀팁임

1. 키워드 분석 활용법
메인 키워드 검색하고 상위 10개 지수 확인
내 지수보다 낮은 키워드부터 공략함
연관 키워드도 같이 봄

2. 내 블로그 분석
매주 같은 요일에 체크함 (변화 추이 파악)
C-Rank, D.I.A 점수 기록해둠
어떤 활동 했을 때 올랐는지 분석
...
이거만 해도 상위노출 확률 올라감""",
    ...
    "question": [
        {
            "title": "블로그 지수 {score} 이면 어느정도임?",
            "content": """블로그 시작한지 {days} 됨
...
비슷한 사람들 조언좀""",
        ...
        {
            "title": "하루에 포스팅 몇개가 적당함?",
            "content": """블로그 열심히 하려고 하는데
하루에 몇개 정도 올려야함?

{days} 동안 {post_count} 정도 올렸는데
더 늘려야하나?

많이 올리면 저품질 걸린다는 말도 있고
매일 올려야 한다는 말도 있고
뭐가 맞는거임?ㅋㅋ
...
고수님들 알려주세요""",
        ...
        {
            "title": "이웃 {neighbor_count}인데 상위노출 가능?",
            "content": """블로그 이웃이 {neighbor_count}밖에 안되는데
상위노출 할수있음?

C-Rank가 이웃수랑 관련있다고 해서 걱정됨
지수는 {score} 정도

이웃 적어도 상위노출 성공한 사람 있어?
어떻게 했는지 궁금""",
            "tags": ["이웃수", "상위노출", "질문"]
        },
        {
            "title": "'{keyword}' 키워드 경쟁 심함?",
            "content": """'{keyword}' 글 쓰려는데
이 키워드 경쟁 심한편임?

블랭크로 분석해보니까 상위 블로그들 지수가 꽤 높던데
내 지수는 {score} 정도인데...

비슷한 키워드 쓰는 사람들 경험 좀 알려줘""",
            "tags": ["키워드경쟁", "키워드분석", "질문"]
        },
        {
            "title": "상위노출 유지가 안됨 ㅠㅠ",
            "content": """며칠전에 '{keyword}'로 {rank} 찍었는데
오늘 보니까 밀렸음 ㅠㅠ
...
경험있는 사람들 조언좀ㅠㅠ""",
    ...
    "success": [
        {
            "title": "'{keyword}' {rank} 찍음 ㅠㅠㅠ 드디어!!",
            "content": """아ㅏㅏㅏ 드디어 첫 상위노출 성공함!!!!

'{keyword}'로 {rank} 찍음 ㅠㅠㅠㅠ

블로그 시작한지 {days}만임
{struggle} 드디어 됐다 {emotion}!
...
나도 했으니까 다들 할수있음!!""",
        ...
        {
            "title": "{level} 달성함!! 성장 과정 공유",
            "content": """오늘 드디어 {level} 달성ㅠㅠㅠ

{struggle} 꾸준히 하니까 되네 {emotion}!
...
다들 화이팅""",
        ...
        {
            "title": "이웃 {neighbor_count} 달성 후기ㅋㅋ",
            "content": """드디어 이웃 {neighbor_count} 됨!!
...
다들 화이팅""",
    ...
    "free": [
        {
            "title": "오늘도 블로그 화이팅ㅋㅋ",
            "content": """오늘도 열심히 포스팅 했음

날씨 좋아서 기분도 좋네
카페에서 {time_spent} 동안 글씀
집중 잘 되는 날이었음

요즘 '{keyword}' 관련 글 열심히 쓰는데
조금씩 반응 오는것 같아서 뿌듯함
...
내일도 화이팅""",
        ...
        {
            "title": "블로그 슬럼프 왔는데 어떡함",
            "content": """요즘 블로그 글쓰기가 진짜 힘듦

{days} 동안 {post_count} 썼는데
뭘 써야할지도 모르겠고
의욕도 안나고...

상위노출도 안되니까 더 의욕 떨어짐 ㅠㅠ
...
다들 주말 잘보내""",
        ...
        {
            "title": "오늘 첫 체험단 선정됨!!",
            "content": """드디어 첫 체험단 선정됐음!!

'{keyword}' 관련 체험단인데
블로그 시작한지 {days} 만에 처음임
...
ANSWER_FILLERS = [
    "한달 정도", "2~3주", "꾸준히 하면 효과 있음",
    "나도 처음엔 힘들었는데", "여러번 시도해봤는데",
    "이방법이 제일 나음", "경험상",
    "나같은경우엔", "걍", "그냥", "ㅋㅋ 나는",
]
