"""
TikTok 인플루언서 검색 클라이언트

경로 A: Google Custom Search site:tiktok.com (1순위)
경로 B: 네이버 웹 검색 site:tiktok.com (2순위)

※ TikTok Research API는 학술/연구 전용, 상업 앱 거절률 높음
"""
import logging
...
class TikTokInfluencerClient(InfluencerPlatformBase):
    PLATFORM_ID = "tiktok"
    ...
    async def search_creators(self, query: str, filters: dict) -> List[InfluencerProfile]:
        """웹 검색으로 TikTok 인플루언서 발견 (Google → Naver)"""
        if not self._has_search:
            logger.info("TikTok: 검색 API 미설정 (Google CSE / Naver)")
            return []
