...
async def get_daily_winners(
    ...
    try:
        ...
        if result["status"] in ("cache_empty", "no_topic_match", "no_topic_terms"):
            message = ("블로그 주제를 아직 파악하지 못했습니다. 글이 쌓이면 주제에 "
                       "맞는 키워드만 골라 보여 드립니다."
                       if result["status"] == "no_topic_terms"
                       else "이 블로그 주제의 키워드를 수집하는 중입니다. 잠시 후 다시 확인해 주세요.")
        else:
...
async def get_quick_winners(
    ...
    try:
        ...
        if result["status"] in ("cache_empty", "no_topic_match", "no_topic_terms"):
            ...
            if result["status"] == "no_topic_terms":
                msg = ("블로그 주제를 아직 파악하지 못했습니다. 글이 쌓이면 "
                       "주제에 맞는 키워드만 골라 보여 드립니다.")
            elif result["status"] == "no_topic_match":
