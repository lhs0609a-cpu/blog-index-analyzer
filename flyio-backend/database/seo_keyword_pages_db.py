...
# 광고축. 이 사이트는 /ad-optimizer 를 팔고, 파워링크 키워드에도 블로그 1페이지가
# 존재하므로 같은 '상위노출 난이도' 페이지가 그대로 성립한다.
#
# ⚠️ 맨 '광고' 를 넣으면 안 된다. '광고차단'·'유튜브광고없이' 처럼 광고를 **없애려는**
# 검색어가 통째로 들어온다 — 우리 독자가 아니다. 그래서 STRONG 은 광고 상품·집행
# 용어로만 좁히고, 제거 의도는 아래 EXCLUDE 에서 따로 막는다.
#
# 실측(2026-09-24): 이 축의 천장은 777개(검색량 100↑ 는 111개)로 89초에 소진됐다.
# 블로그 메타축(2,055/383)보다 작다 — 여기에 '1만 개'는 없다.
DOMAIN_TOKENS_AD = (
...
AD_EXCLUDE = ("광고차단", "광고제거", "광고지우기", "광고없이", "광고스킵", "팝업차단")
...
def in_domain(keyword: str) -> bool:
    ...
    if any(x.lower() in k for x in DOMAIN_EXCLUDE):
        return False
    if any(x.lower() in k for x in AD_EXCLUDE):
        return False
