...
# v5: 가중치 해석 단일화. 붕괴한 레거시 학습값(c_rank 0.054)을 버리고
#     카테고리 사전값 + 하한/상한 재정규화로 채점. v4 점수와는 자가 다르다.
# v6: 콘텐츠 표본 3 → 15. 콘텐츠는 총점 가중치 50% 인데 그 값이 최신 글 3개의
#     평균이었다. 하루 6~7개씩 올리는 블로그는 **매일 표본이 통째로 갈려서**
#     블로그가 그대로여도 점수가 ±8~11 씩 튀었다(실측 platonmarketing).
#     글 단위 캐시를 붙여 표본을 키웠다.
#     ★같은 블로그 실측: v5 72.9(준최5) → v6 82.5(최적1). 블로그는 그대로다 —
#      자가 바뀐 것이다. 버전을 올리지 않으면 이 +9.6 이 그래프에 '상승' 으로
#      찍히고, 우리가 코드를 고친 것을 사용자 블로그가 좋아진 것으로 말하게 된다.
SCORING_VERSION = 6
