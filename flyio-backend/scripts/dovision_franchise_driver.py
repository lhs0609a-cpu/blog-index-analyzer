...
dovision_franchise_seeds.json 을 25개씩 배치로 seed-explode-register(min_volume=20, min_score=0)
...
CID = "4403292"
seeds = json.load(open(os.path.join(HERE, "dovision_franchise_seeds.json"), encoding="utf-8"))["seeds"]
...
print(f"[프랜차이즈] 발굴 시작 — {len(seeds):,} 시드 → {len(batches):,} 배치(×{CH})", flush=True)
p0, r0, _ = stats()
...
for bi, chunk in enumerate(batches, 1):
    ...
    time.sleep(18)
pf, rf, prf = stats()
print(f"[프랜차이즈] 발굴 완료 — 최종 pool={pf} registered={rf} (pool내 registered={prf})", flush=True)
